    ...
    case (instr.i_type.opcode)
      ...
      riscv::OpcodeSystem: begin
        ...
        endcase
            ...
            // Custom accelerator configuration and instruction
      7'b0101011: begin // custom-1 for mpcfg
        // Only accept when rd != x0 (写回VRF)
        is_accel_o = 1'b1;
        is_rd      = 0;
      end

      7'b0001011: begin // custom-0 for mple, mpse, mpmm
        // Decode based on funct3 field
        case (instr.i_type.funct3)
          3'b010: begin // mple
            is_accel_o = 1'b1;
            is_rs1     = 1'b1;  // base address
            is_rd      = 1'b0;  // 写入VRF
            is_load    = 1'b1;  // load
          end
          3'b100: begin // mpse
            is_accel_o = 1'b1;
            is_rs1     = 1'b1;  // base address
            is_rd      = 1'b0;  // VRF -> memory
            is_store   = 1'b1;  // store
          end
          3'b110: begin // mpmm (pseudo op, single trigger)
            is_accel_o = 1'b1;
            is_rd      = 1'b0;
          end
          default: ; // 非法指令（保持默认 illegal）
        endcase
      end
    endcase
  end
