...
static DecodeStatus
decodeRVCInstrRdRs1ImmZero(MCInst &Inst, uint32_t Insn, uint64_t Address,
                           const MCDisassembler *Decoder)
{
                                ...
                                const MCDisassembler *Decoder)
{
    ...
    // 验证funct3匹配（确保是MPLE指令）
    if (funct3 != 0b010)
        return MCDisassembler::Fail;

    // 解码标志位和偏移量
    bool isWeight = (imm >> 11) & 0x1;
    int32_t offset = SignExtend32<11>(imm & 0x7FF);

    // 构建MCInst（操作数顺序需与TableGen定义严格一致）
    DecodeGPRRegisterClass(Inst, Rs1, Address, Decoder);
    Inst.addOperand(MCOperand::createImm(offset));   // 偏移量
    Inst.addOperand(MCOperand::createImm(isWeight)); // w=1, a=0
                                ...
                                const MCDisassembler *Decoder)
{
    ...
    // 构建MCInst（操作数顺序需与TableGen定义严格一致）
    DecodeGPRRegisterClass(Inst, Rs1, Address, Decoder);
    Inst.addOperand(MCOperand::createImm(imm)); // 偏移量
