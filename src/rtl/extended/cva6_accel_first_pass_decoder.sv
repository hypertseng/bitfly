...
#(
  ...
  always_comb begin
    ...
    case (instr.i_type.opcode)
      ...
      riscv::OpcodeLoadFp, riscv::OpcodeStoreFp: begin
        case ({
      ...
      //-------------------------------
      // SA 加载指令 (复用 Load-FP 操作码)
      //-------------------------------
      riscv::OpcodeLoadFp: begin
        if (instr.vmem_type.mew == 1'b1 && instr.vmem_type.width == 3'b000) begin
          is_accel_o = 1'b1;
          is_load    = 1'b1;
          is_rs1     = 1'b1;  // 基地址寄存器 rs1
          is_rd      = 1'b1;  // 目标寄存器 rd
        end
      end

      //-------------------------------
      // SA 存储指令 (复用 Store-FP 操作码)
      //-------------------------------
      riscv::OpcodeStoreFp: begin
        if (instr.vmem_type.mew == 1'b1 && instr.vmem_type.width == 3'b000) begin
          ...
          is_store   = 1'b1;
          is_rs1     = 1'b1;  // 基地址寄存器 rs1
          is_rs2     = 1'b1;  // 数据寄存器 rs2
        end
      end

      //-------------------------------
      // SA 计算指令 (使用 Custom-0 操作码)
      //-------------------------------
      riscv::OpcodeCustom0: begin
        if (instr.rtype.funct7 == 7'b1000000) begin
          is_accel_o = 1'b1;
          is_rs1     = 1'b1;  // 输入激活值寄存器 rs1
          is_rs2     = 1'b1;  // 权值寄存器 rs2
          is_rd      = 1'b1;  // 结果寄存器 rd
        end
      end
    endcase
  ...
  always_comb begin
    ...
    if (is_accel_o && vs_i != riscv::Off) begin // trigger illegal instruction if the vector extension is turned off
      ...
      // 设置操作类型
      unique case (1'b1)
        is_load:  instruction_o.op = ACCEL_OP_SA_LD;
        is_store: instruction_o.op = ACCEL_OP_SA_ST;
        default:  instruction_o.op = ACCEL_OP_SA_CALC;  // SA.CALC
      endcase
      ...
      case (instr.i_type.opcode)
        riscv::OpcodeLoadFp, riscv::OpcodeStoreFp: begin
          if (instr.vmem_type.mew != 1'b1 || instr.vmem_type.width != 3'b000)
            illegal_instr_o = 1'b1;
        end
        riscv::OpcodeCustom0: begin
          if (instr.rtype.funct7 != 7'b1000000) illegal_instr_o = 1'b1;
        end
      endcase

      // Decode the vector operation
      unique case ({
        is_store, is_load, is_fs1, is_fs2, is_fd
      })
