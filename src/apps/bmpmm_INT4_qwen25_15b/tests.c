...
extern void int4_mixed_matmul(int16_t *c, const int8_t *a, const int8_t *b,
                              const unsigned long int M, const unsigned long int K,
                              const unsigned long int N);
extern void vector_int4_matmul(int16_t *restrict c, const int8_t *restrict a, const int8_t *restrict b,
...
void run_test(const char *test_name, int M, int K, int N)
{
    ...
    {
        int4_mixed_matmul(data.result_lp, data.activation_lp, data.weight_lp, M, K, N);
    }
    else if (strcmp(test_name, "vector") == 0)
    {
        vector_int4_matmul(data.result_hp, data.activation_hp, data.weight_hp, M, K, N);
    }
    else
    {
