...
int main()
{
    printf("[rvv_INT4] precision=int4\n");
    RVV_LOG_FLUSH();
    const char *current_model = 0;
    int64_t rvv_model_cycles = 0;
    const int fast_mode = (vector_int4_get_default_mode() == RVV_INT4_VECTOR_EXEC_FAST);
    ...
    for (int i = 0; i < BMPMM_BENCH_CASE_COUNT; ++i)
    {
        ...
        if (!current_model || strcmp(current_model, sc->model) != 0)
        {
            if (current_model)
            {
                printf("[rvv_INT4] model_total model=%s rvv_cycles=%ld\n", current_model, (long)rvv_model_cycles);
                RVV_LOG_FLUSH();
            }
            current_model = sc->model;
            rvv_model_cycles = 0;
            printf("\n============================================================\n");
            printf("[rvv_INT4] model=%s scale=%s\n", sc->model, sc->scale);
            printf("============================================================\n");
            RVV_LOG_FLUSH();
        }
        printf("\n------------------------------------------------------------\n");
        printf("[rvv_INT4] case%d layer=%s shape=(%lu,%lu,%lu)\n", i+1, sc->layer, sc->M, sc->N, sc->K);
        RVV_LOG_FLUSH();
        ...
        start_timer();
        vector_int4_matmul(data.result_hp, data.activation_hp, data.weight_hp, sc->M, sc->K, sc->N);
        stop_timer();
        int64_t rvv_runtime = fast_mode ? vector_int4_get_last_estimated_total_cycles() : get_timer();
        int64_t rvv_compute = fast_mode ? vector_int4_get_last_estimated_compute_cycles() : vector_compute_time;
