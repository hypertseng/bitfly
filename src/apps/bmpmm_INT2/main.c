...
BMPMM_DEFINE_BENCH_CASES_G(kBenchCases, 2UL, 64UL, 2UL, 1UL, 2UL, 1UL);
...
int main()
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        printf("\n------------------------------------------------------------\n");
        printf("[bmpmm_INT2] case%d shape=(%lu,%lu,%lu), cfg=(mt=%lu,nt=%lu,kt=%lu,gm=%lu,gn=%lu,p=%lu)\n",
               i + 1, sc->M, sc->N, sc->K,
               sc->cfg.mtile, sc->cfg.ntile, sc->cfg.ktile,
               sc->cfg.gm, sc->cfg.gn, sc->cfg.prec);

        start_timer();
        int ok = int2_mixed_matmul_with_cfg(
        ...
        if (!ok)
        {
            printf("[bmpmm_INT2] ERROR: execution failed for case%d\n", i + 1);
            continue;
        }
        ...
        printf("[bmpmm_INT2] runtime=%ld cycles, perf=%.2f OP/cycle, util=%.2f%%\n",
               (long)runtime, performance, utilization);
        printf("[bmpmm_INT2] sample C(col-major) = {%d, %d, %d, %d}\n",
