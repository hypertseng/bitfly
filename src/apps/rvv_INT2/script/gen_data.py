...
def pack_weights_int2(weight_mat):
...
def build_square_dataset(lines, s):
    ...
    d = math.ceil(s / 8)
    lines.append(f"/* square={s}: int2, depth={d}, layout=8x16 bitplanes */")
    ...
    if SKIP_LP_DATA:
        ...
        weight_words, _ = pack_weights_int2(weight)
...
def build_top_shape_dataset(lines, case):
    ...
    _dbg(f"[rvv_int2_gen] begin {name} shape=({m_dim},{n_dim},{k_dim})")
    ...
    _dbg(f"[rvv_int2_gen] arrays_ready {name} hp_m={hp_m_dim} hp_k={hp_k_dim}")
    ...
    _dbg(f"[rvv_int2_gen] done {name}")


def main():
    parser = argparse.ArgumentParser(description="Generate RVV INT2 datasets")
