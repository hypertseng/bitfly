...
#define RVV_INT2_DO_VWMACC_PROFILED(dst, scalar, src) \
    do                                                \
    {                                                 \
        int64_t start = get_cycle_count();            \
        RVV_INT2_DO_VWMACC_RAW(dst, scalar, src);     \
        vector_compute_time += get_cycle_count() - start; \
    } while (0)

#define RVV_INT2_MATMUL_VEC_BODY(VWMACC_OP)                      \
    do                                                           \
    {                                                            \
                                      ...
                                      unsigned long int M, unsigned long int K, unsigned long int N)
{
    ...
    for (unsigned long int n = 0; n < N; n += block_size_n)
    {
        ...
        for (unsigned long int m = 0; m < M; m += block_size)
        {
            ...
            rvv_int2_matmul_vec_profiled(c__, a_, b_, K, N);
        }
    }
}

static void rvv_int2_measure_block(int16_t *restrict c, const int8_t *restrict a, const int8_t *restrict b,
                                     ...
                                     int64_t *block_total_cycles, int64_t *block_compute_cycles)
{
    ...
    rvv_int2_matmul_vec_unprofiled(c, a, b, K, N);
                                    ...
                                    int64_t *estimated_total_cycles)
{
...

    {
        ...
        if (full_slices != 0UL)
        {
            rvv_int2_measure_block(c, a, full_b, K, N, sample_n,
        ...
        if (tail_n != 0UL)
        {
            rvv_int2_measure_block(c, a, b + full_slices * sample_n, K, N, tail_n,
...
static void rvv_int2_matmul_vec_profiled(int16_t *c, const int8_t *a, const int8_t *b,
                                         const unsigned long int K, const unsigned long int N)
{
    RVV_INT2_MATMUL_VEC_BODY(RVV_INT2_DO_VWMACC_PROFILED);
}

static void rvv_int2_matmul_vec_unprofiled(int16_t *c, const int8_t *a, const int8_t *b,
                                           const unsigned long int K, const unsigned long int N)
{
    RVV_INT2_MATMUL_VEC_BODY(RVV_INT2_DO_VWMACC_RAW);
}
