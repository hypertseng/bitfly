...
static int64_t rvv_int4_measure_sample_rows_cycles(int16_t *restrict scratch_buf,
                                                   ...
                                                   unsigned long int N)
{
    ...
    if (scratch_buf)
    {
        int4_reference_matmul(scratch_buf, a, b, sample_rows, K, N);
    }
    else
    {
        int16_t scratch[sample_rows * N];
        int4_reference_matmul(scratch, a, b, sample_rows, K, N);
    }
                                    ...
                                    int64_t *estimated_total_cycles)
{
    ...
    const int64_t sample_cycles = rvv_int4_measure_sample_rows_cycles(c, a, b, sample_rows, K, N);
